...
func NewRegistry(root string, opts Options) *Registry {
	...
	r := &Registry{
		...
		audit:         AuditLog{Dir: filepath.Join(config.StateDir(), "audit")},
	}
	r.registerBuiltins()
	return r
}
...
func (r *Registry) Execute(ctx context.Context, call llm.ToolCall) Result {
	...
	if tool.Dangerous || strings.HasPrefix(name, "mcp__") {
		// Dangerous local tools and all MCP calls are audit-recorded with a hash
		// of arguments. This keeps logs useful without leaking file content,
		// command payloads, or remote tool secrets into plaintext audit files.
...
func (r *Registry) registerFileTools() {
	...
	r.register(Tool{
		...
		Executor: func(ctx context.Context, args map[string]any) (string, error) {
			...
			if desc == "" {
				desc = "Generated by MiniOpsAgent."
			}
...
func (r *Registry) registerRAGTools() {
	r.register(Tool{
		...
		Executor: func(ctx context.Context, args map[string]any) (string, error) {
			...
			if len(results) == 0 {
				return "no indexed matches; run `miniopsagent index` first or use grep_code", nil
			}
			var b strings.Builder
...
func (r *Registry) registerMemorySkillTools() {
	r.register(Tool{
		Name:        "save_memory",
		Description: "Save a stable fact only when the user explicitly asks MiniOpsAgent to remember it.",
...
func shellCommand(ctx context.Context, command string) *exec.Cmd {
	// The agent runs where the operator is working. Use PowerShell on Windows
	// and POSIX shell elsewhere so execute_command works in both local labs and
	// Linux servers without forcing WSL/Git Bash as a hidden dependency.
